...
class BanditRouter:
    ...
    def _get_or_create(self, name: str, arms: list | None = None) -> Bandit:
    ...
    def select(self, name: str, arms: list | None = None) -> int:
        bandit = self._get_or_create(name, arms)
        idx = bandit.select_arm()
        return idx

    def select_arm_value(self, name: str, arms: list | None = None):
        bandit = self._get_or_create(name, arms)
        idx = bandit.select_arm()
    ...
    @classmethod
    def load(cls, path: str | Path, default_bandit_cls=EpsilonGreedyBandit, **default_kwargs):
